...
All formatting attributes are prohibited (`align`, `alink`, `background`, `bgcolor`, `border`, `cellpadding`, `cellspacing`, `char`, `charoff`, `clear`, `compact`, `color`, `frameborder`, `hspace`, `link`, `marginheight`, `marginwidth`, `text`, `valign`, `vlink`, `vspace`, `size`).<br>`width` and `height` are only allowed on elements `<img>`, `<svg>`, `<object>`, `<embed>` and `<canvas>`.
</div>
